...
def create_order():
    ...
    if request.method == "POST":
        with Session() as db_session:
            ...
            for menu, qty in zip(menu_items, quantities):
                db_session.add(OrderMenu(order_id=order.id, menu_id=menu.id, quantity=int(qty)))
    ...
    return render_template("account/basket.html", basket=dict(zip(menu_items, quantities)))
...
def order_details(order_id):  
    ...
    return render_template("account/orders_details.html", 
                           order=order, 
